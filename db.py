    ...
    def _init_sync(self) -> None:
        with self._connect() as conn:
            ...
            if count == 0:
                conn.executemany(
                    """
                    INSERT INTO anime(title_ru, title_en, description, genres, year, rating)
                    VALUES (:title_ru, :title_en, :description, :genres, :year, :rating)
                    """,
                    [{**a, "genres": ",".join(a["genres"])} for a in ANIME_DATA],
                )
            conn.commit()
